...
def get_nearest_neighbors(example_set, query, k):
    difference = example_set - query # broadcasting
    distance = np.linalg.norm(difference, ord=2, axis=1) # calculate the distance
    k_nearest = np.argsort(distance) # use argsort to make sure that it runs efficiently
    idx_of_nearest = k_nearest[:k]
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    for i in range(len(k_nearest)):
        k_nearest_labels.append(examples_y[k_nearest[i]][-1]) # this indexing is used to grab the specific array and then the last element of it
    predicted_label = mode(k_nearest_labels) # use statistics library to obtain our guess based on the nearest neighbors
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    ...
    example_labels = []
    all_accuracies = []
    example_sub_arrays = np.split(train_X, num_folds) # split the arrays
    example_label_sub_arrays = np.split(train_y, num_folds)
    for i in range(num_folds):
        test_data = example_sub_arrays[i].tolist() # cast these as lists so that comparision can be done
        test_labels = example_label_sub_arrays[i].tolist()
        example_labels = np.vstack([l for j,l in enumerate(example_label_sub_arrays) if j != i]) # stack all of the matricies into one
        example_data = np.vstack([l for j,l in enumerate(example_sub_arrays) if j != i])
        k = min(k, len(example_data)) # handle the edge case of k = 8000. we cannot compare the 8,000 nearest neighbors of 6,000 total points
        predicted_labels = predict(example_data, example_labels, test_data, k)
        accuracy = compute_accuracy(test_labels, predicted_labels)
        all_accuracies.append(accuracy) # append the current accuracy to the list of all accuracies

    return np.mean(all_accuracies), np.var(all_accuracies)
