...
def main():
  ...
  w, bias_losses = trainLogistic(X_train_bias,y_train)
  actual_predictions = X_test_bias@w >= 0
  
  # This portion was taken from assignment 1
  test_out = np.concatenate((np.expand_dims(np.array(range(len(actual_predictions)),dtype=np.int), axis=1), actual_predictions), axis=1)
...
######################################################################
def logistic(z):
  return 1 / (1 + np.exp(0.000001 + np.negative(z)))
...
def calculateNegativeLogLikelihood(X,y,w):
  nll = -(y * np.log(0.000001 + logistic(X@w)) + (1 - y) * np.log((1 + 0.0000001) - logistic(X@w)))
  return nll[0]
...
def dummyAugment(X):
  rows, cols = X.shape
  col_of_ones = np.ones(shape=(rows, 1))
  aug_X = np.hstack((col_of_ones, X))
  return aug_X
