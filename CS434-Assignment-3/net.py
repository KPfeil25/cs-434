...
def main():
  ...
  net = FeedForwardNeuralNetwork(X_train.shape[1],10,width_of_layers,number_of_layers, activation=True)
  ...
  acc = np.argmax(logits, axis=1)[:,np.newaxis]
  # This portion was taken from assignment 1
  test_out = np.concatenate((np.expand_dims(np.array(range(len(X_test)),dtype=np.int), axis=1), acc), axis=1)
