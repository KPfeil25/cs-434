...
def imageProblem():
  ...
  print("Final SSE for k = 4: " + str(SSE[-1]))
  # Visualize Clusters
...
def initalizeCentroids(dataset, k):
  ...
  for _ in range(k):
    ...
    centroids.append(dataset[rand_idx])
...
def computeAssignments(dataset, centroids):
  ...
  for i in range(len(dataset)):
    ...
    for j in range(len(centroids)):
      intermediate_distance = (np.linalg.norm([dataset[i] - centroids[j]]))
      if intermediate_distance < min_distance:
        min_idx = j
        min_distance = intermediate_distance
...
def updateCentroids(dataset, centroids, assignments):
  ...
  for i in range(len(centroids)):
    ...
    if not curr_count == 0:
      centroids[i] = (centroids[i] / curr_count)
...
def calculateSSE(dataset, centroids, assignments):
  ...
  sse = 0
  for i in range(len(dataset)):
    dist = (np.linalg.norm(dataset[i] - centroids[assignments[i]])**2)
    sse += float(dist)
  return sse
